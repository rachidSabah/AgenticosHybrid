...
class WingetDiscovery(DiscoveryProvider):
    ...
    _known_packages: tuple[dict[str, Any], ...] = field(
        default_factory=lambda: (
            {"name": "claude-code", "pid": "Anthropic.ClaudeCode", "engine": EngineType.CLAUDE_CODE,
             "caps": [EngineCapability.CODING, EngineCapability.REASONING, EngineCapability.TERMINAL]},
            {"name": "opencode", "pid": "Sentry.OpenCodeCLI", "engine": EngineType.OPENCODE,
             "caps": [EngineCapability.CODING, EngineCapability.REASONING]},
            {"name": "ollama", "pid": "Ollama.Ollama", "engine": EngineType.CUSTOM,
             "caps": [EngineCapability.REASONING, EngineCapability.OFFLINE]},
            {"name": "git", "pid": "Git.Git", "engine": EngineType.CUSTOM,
             "caps": [EngineCapability.GIT]},
            {"name": "openinterpreter", "pid": "OpenInterpreter.OpenInterpreter", "engine": EngineType.OPEN_INTERPRETER,
    ...
    async def discover(self) -> list[EngineRegistration]:
        ...
        try:
            result = subprocess.run(["winget", "list", "--accept-source-agreements"],
                                    capture_output=True, timeout=30.0, text=True)
    ...
    def get_provider_name(self) -> str: return "winget-discovery"
    def get_provider_type(self) -> str: return "winget"
