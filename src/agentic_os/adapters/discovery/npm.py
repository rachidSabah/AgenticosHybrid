...
class NpmDiscovery(DiscoveryProvider):
    ...
    _known_packages: tuple[dict[str, Any], ...] = field(
        default_factory=lambda: (
            {"name": "claude-code", "package": "@anthropic-ai/claude-code", "engine": EngineType.CLAUDE_CODE,
             "caps": [EngineCapability.CODING, EngineCapability.REASONING, EngineCapability.TERMINAL]},
            {"name": "opencode", "package": "@sentry/opencode-cli", "engine": EngineType.OPENCODE,
             "caps": [EngineCapability.CODING, EngineCapability.REASONING, EngineCapability.TERMINAL]},
            {"name": "codex", "package": "@openai/codex-cli", "engine": EngineType.CODEX,
             "caps": [EngineCapability.CODING, EngineCapability.REASONING]},
            {"name": "aider", "package": "aider", "engine": EngineType.AIDER,
             "caps": [EngineCapability.CODING, EngineCapability.PLANNING]},
            {"name": "hermes", "package": "@hermes-agents/hermes", "engine": EngineType.HERMES,
             "caps": [EngineCapability.CODING, EngineCapability.REASONING, EngineCapability.PLANNING, EngineCapability.TERMINAL]},
            {"name": "qwen", "package": "@alibaba/qwen-cli", "engine": EngineType.QWEN,
             "caps": [EngineCapability.CODING, EngineCapability.REASONING]},
        )
    )
            ...
            for entry in self._known_packages:
                ...
                if pkg_name in deps:
                    ...
                    results.append(EngineRegistration(
                        ...
                        description=f"{entry['name'].title()} (npm global)" + (f" v{version}" if version else ""),
    ...
    def get_provider_name(self) -> str: return "npm-discovery"
    def get_provider_type(self) -> str: return "npm"
