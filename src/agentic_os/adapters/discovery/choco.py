...
class ChocolateyDiscovery(DiscoveryProvider):
    ...
    _known_packages: tuple[dict[str, Any], ...] = field(
        default_factory=lambda: (
            {"name": "claude-code", "package": "claude-code", "engine": EngineType.CLAUDE_CODE,
            ...
            {"name": "ollama", "package": "ollama", "engine": EngineType.CUSTOM,
    ...
    async def discover(self) -> list[EngineRegistration]:
        ...
        choco_dir = os.path.join(os.environ.get("ChocolateyInstall", "C:\\ProgramData\\chocolatey"), "bin")
        ...
        for entry in self._known_packages:
            ...
            results.append(EngineRegistration(
                ...
                description=f"{entry['name'].title()} (Chocolatey)" + (f" v{version}" if version else ""),
    ...
    def get_provider_name(self) -> str: return "chocolatey-discovery"
    def get_provider_type(self) -> str: return "chocolatey"
