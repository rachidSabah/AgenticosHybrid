...
def _completed(output: bytes, returncode: int = 0) -> subprocess.CompletedProcess[bytes]:
    return subprocess.CompletedProcess(args=[], returncode=returncode, stdout=output, stderr=b"")
...
class TestProcessScannerWindows:
    ...
    async def test_windows_returns_results(self, scanner: ProcessScanner) -> None:
        ...
        with patch("subprocess.run", return_value=_completed(tasklist_output.encode("utf-8"))):
    ...
    async def test_windows_tasklist_timeout(self, scanner: ProcessScanner) -> None:
        with patch("subprocess.run", side_effect=subprocess.TimeoutExpired("tasklist", 10.0)):
            results = await scanner.scan()
            assert results == []
...
class TestProcessScannerPosix:
    ...
    async def test_posix_returns_results(self, scanner: ProcessScanner) -> None:
        ...
        with patch.object(scanner, "_system", "linux"):
            with patch("subprocess.run", return_value=_completed(ps_output.encode("utf-8"))):
                results = await scanner.scan()
                assert len(results) == 2
    ...
    async def test_posix_ps_timeout(self, scanner: ProcessScanner) -> None:
        with patch("subprocess.run", side_effect=subprocess.TimeoutExpired("ps", 10.0)):
            results = await scanner.scan()
            assert results == []
