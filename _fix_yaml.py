...
for i in range(190, min(193, len(lines))):
    stripped = lines[i].lstrip()
    current_indent = len(lines[i]) - len(stripped)
    if current_indent == 8 and (
        stripped.startswith("Write") or stripped.startswith("Start") or stripped.startswith("'@")
    ):
        lines[i] = "          " + stripped
...
for i in range(343, min(400, len(lines))):
    ...
    if current_indent == 8 and (
        ...
        or stripped.startswith('"releaseDate"')
        or stripped.startswith('"commit"')
        or stripped.startswith('"platforms"')
        or stripped.startswith('"windows"')
        or stripped.startswith('"os"')
        or stripped.startswith('"arch"')
        or stripped.startswith('"installers"')
