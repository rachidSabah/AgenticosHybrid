  ...
  return (
    <div className="scroll-page">
      <div className="rflex gap-4 p-4">
        <Panel title="Providers" subtitle={`${rows.length} registered`} className="flex-[5]">
        ...
        <Panel title="Provider Detail" subtitle={selected ?? "Select a provider"} className="flex-[7]">
          {!selected ? (
              ...
              <div>
                ...
                <div className="card-grid">
                  {(models[selected] ?? []).map((m) => (
                    <div key={m.id} className="card-fluid rounded-xl border border-border/60 px-3 py-2">
                  ...
                  {(models[selected] ?? []).length === 0 && (
                    <div className="col-span-full text-sm text-faint">No models exposed for this provider.</div>
                  )}
                </div>
              </div>
            </div>
