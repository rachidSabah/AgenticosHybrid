  ...
  <iframe src="http://localhost:8080/preview/" title="Cabin Crew Morocco Live Preview"></iframe>
    ...
    try {
      ...
      if (mission?.id) {
        ...
        // ── Real Execution Monitoring (spec §1, §2, §7, §13) ──
        const asstMsgId = `msg-asst-${Date.now()}`;
        const initialAsstMsg: ChatMessage = {
          id: asstMsgId,
          role: "assistant",
          content: `Mission dispatched [${mission.id.slice(0, 8)}]. Orchestrator assigned priority [${mission.priority || "high"}] across runtime agents. Awaiting execution and independent artifact verification...`,
          timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
          artifacts: userArtifacts.length > 0 ? userArtifacts : undefined,
        };
        setMessages((prev) => [...prev, initialAsstMsg]);

        // Monitor real task execution and verified deliverables asynchronously
        (async () => {
          let attempts = 0;
          ...
          while (attempts < maxAttempts) {
            ...
            attempts++;
            try {
              const missionData = await api.getMission(mission.id);
              const mStatus = missionData?.status;
              const allTasks = await api.listTasks();
              const missionTasks = (allTasks || []).filter((t: any) => t.mission_id === mission.id);
              ...
              if (
                ...
                const anyFailed = mStatus === "failed" || missionTasks.some((t: any) => t.status === "failed");
                const verifiedArtifacts: Artifact[] = [...userArtifacts];

                missionTasks.forEach((t: any) => {
                  if (Array.isArray(t.artifacts)) {
                    t.artifacts.forEach((art: any) => {
                      const relPath = String(art.path || art);
                      const artObj: Artifact = {
                        id: `art-${t.id}-${relPath.replace(/[^a-zA-Z0-9_-]/g, "_")}`,
                        messageId: asstMsgId,
                        title: relPath,
                        type:
                        ...
                        content:
                          art.content ||
                          `/* Deliverable independently verified on disk: ${relPath} (${art.size || 0} bytes) */`,
                        language: relPath.split(".").pop() || "text",
                        status: "ready",
                        version: 1,
                        timestamp: new Date().toISOString(),
                        filePath: relPath,
                      ...
                      verifiedArtifacts.push(artObj);
                    });
                  }
                });

                if (verifiedArtifacts.length > 0) {
                  setActiveArtifactId(verifiedArtifacts[verifiedArtifacts.length - 1].id);
                  setIsArtifactOpen(true);
                }
                ...
                if (anyFailed) {
                  const errDetails =
                    missionTasks
                      .map((t: any) => t.error)
                      .filter(Boolean)
                      .join("\n") || "Mission execution or verification failed.";
                  finalContent = `❌ **FAILED_VERIFICATION / EXECUTION FAILED**\n\n${errDetails}\n\nDeliverable verification failed: real expected files were missing or empty on disk.`;
                } else {
                  const agentNames =
                    missionTasks
                      .map((t: any) => t.assigned_agent_id)
                      .filter(Boolean)
                      .join(", ") || "runtime agent";
                  finalContent = `✅ **EXECUTION COMPLETED & INDEPENDENTLY VERIFIED**\n\nExecuted by: \`${agentNames}\`\nVerified Deliverables: ${verifiedArtifacts.length} artifact(s) on disk.`;
                  if (missionTasks[0]?.result) {
                    finalContent += `\n\n${missionTasks[0].result}`;
                  }
                }
                  ...
                  prev.map((msg) =>
                    msg.id === asstMsgId
                      ? {
                          ...
                          artifacts: verifiedArtifacts.length > 0 ? verifiedArtifacts : undefined,
                        }
                      : msg
                  )
                );
